...
export function PopularCityList() {
  ...
  const handleRedirect = (city: string) => {
    ...
    router.push(
      `/create-new-trip?destination=${encodeURIComponent(destination)}`
    );
  };
      ...
      >
        {data.map((dest) => (
          <div
            ...
            onClick={() => handleRedirect(dest.city)}
          >
            <img
              src={dest.src}
              alt={dest.city}
            ...
            <div className="absolute bottom-0 left-0 right-0 p-5 translate-y-8 group-hover:translate-y-0 transition-transform duration-300">
              ...
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  handleRedirect(dest.city);
                }}
                className="w-full bg-white text-gray-900 text-xs font-semibold tracking-wide py-2.5 rounded-xl opacity-0 group-hover:opacity-100 transition-all duration-300 hover:bg-orange-500 hover:text-white"
              >
                Plan a Trip to {dest.city.split(",")[0]}
              </button>
            </div>
          </div>
        ))}
...
const data = [
  ...
  },
  {
    category: "India",
    city: "Agra, India",
    title: "Discover Incredible India – Taj Mahal and more",
    duration: "3–5 days",
    budget: "₹20k–₹50k",
    bestTime: "Oct–Mar",
    emoji: "🏯",
