...
export const suggestions = [
  {
    ...
    value: "Plan a complete vacation for me",
    icon: <TreePalm className="text-green-500 h-4 w-4" />,
  },
  {
    title: "Find the best hotels",
    value: "Suggest the best hotels for my trip",
    icon: <Hotel className="text-blue-500 h-4 w-4" />,
  },
  {
    title: "Budget friendly trip",
    value: "Plan a budget friendly trip for me",
    icon: <HandCoins className="text-orange-500 h-4 w-4" />,
  },
  {
    title: "Weekend getaway",
    value: "Suggest a weekend getaway trip",
    icon: <CableCar className="text-red-400 h-4 w-4" />,
  },
];
...
function Hero() {
  ...
  const handleNavigate = (prompt?: string) => {
    ...
    if (prompt && prompt.trim()) {
      router.push(`/create-new-trip?prompt=${encodeURIComponent(prompt)}`);
    } else {
      ...
      <div className="max-w-3xl w-full space-y-8 relative z-10">
        ...
        <div className="text-center space-y-2">
          ...
          <p className="text-lg text-gray-500 pt-3 font-normal max-w-lg mx-auto leading-relaxed">
            Your personal travel genie. Describe your trip and get a full itinerary instantly.
          </p>
        </div>
        ...
        <div className="grid grid-cols-3 gap-4">
          {[
            ...
            <div
              key={label}
              className="text-center py-4 px-3 bg-gray-50 rounded-xl border border-gray-100"
            >
